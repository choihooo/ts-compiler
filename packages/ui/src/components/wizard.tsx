/**
 * Wizard component
 */

import type { DeepPartial, DeepReadonly, Path, PathValue } from '@bench/core';
import type { FC, ComponentType, ComponentProps, PropsWithChildren, Ref } from '../types/react';
import type { PolymorphicComponentProps, AsProp } from '../types/component';
import type { StyledProps, InteractiveProps, AccessibleProps } from '../types/props';

export type WizardProps = StyledProps<
  AccessibleProps<
    InteractiveProps<
      PropsWithChildren<{
        readonly variant?: 'default' | 'primary' | 'secondary';
        readonly size?: 'small' | 'medium' | 'large';
        readonly disabled?: boolean;
      }>
    >
  >
>;

export const Wizard: FC<WizardProps> = (props) => {
  return { type: 'div', props };
};

export type PolymorphicWizardProps<C extends ComponentType<any> = ComponentType<any>> =
  PolymorphicComponentProps<C, WizardProps & AsProp<C>>;

export function PolymorphicWizard<C extends ComponentType<any> = ComponentType<any>>(
  props: PolymorphicWizardProps<C>
): { type: C; props: PolymorphicWizardProps<C> } {
  return { type: (props.as as C) || ('div' as any), props };
}
