...
/**
 * Additional utility type 33
 */

export type Utility33Base<T> = {
  readonly value: DeepReadonly<T>;
  readonly path?: Path<T>;
};

export type Utility33Options<T = any> = DeepPartial<Utility33Base<T>>;

export type Utility33Result<T> = IsPromise<T> extends true
  ? Promise<DeepReadonly<PromiseValue<T>>>
  : DeepReadonly<T>;

export type Utility33Transform<T, U = T> = {
  readonly input: DeepReadonly<T>;
  readonly output: DeepReadonly<U>;
  readonly transformed: boolean;
};

export function utility33<T>(value: T): Utility33Result<T> {
  return value as Utility33Result<T>;
}

export function transformUtility33<T, U = T>(
  input: T,
  transform?: (value: DeepReadonly<T>) => U
): Utility33Transform<T, U> {
  return {
