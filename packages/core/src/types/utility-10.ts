...
/**
 * Additional utility type 10
 */

export type Utility10Base<T> = {
  readonly value: DeepReadonly<T>;
  readonly path?: Path<T>;
};

export type Utility10Options<T = any> = DeepPartial<Utility10Base<T>>;

export type Utility10Result<T> = IsPromise<T> extends true
  ? Promise<DeepReadonly<PromiseValue<T>>>
  : DeepReadonly<T>;

export type Utility10Transform<T, U = T> = {
  readonly input: DeepReadonly<T>;
  readonly output: DeepReadonly<U>;
  readonly transformed: boolean;
};

export function utility10<T>(value: T): Utility10Result<T> {
  return value as Utility10Result<T>;
}

export function transformUtility10<T, U = T>(
  input: T,
  transform?: (value: DeepReadonly<T>) => U
): Utility10Transform<T, U> {
  return {
