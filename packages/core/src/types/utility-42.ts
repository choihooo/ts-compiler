...
/**
 * Additional utility type 42
 */

export type Utility42Base<T> = {
  readonly value: DeepReadonly<T>;
  readonly path?: Path<T>;
};

export type Utility42Options<T = any> = DeepPartial<Utility42Base<T>>;

export type Utility42Result<T> = IsPromise<T> extends true
  ? Promise<DeepReadonly<PromiseValue<T>>>
  : DeepReadonly<T>;

export type Utility42Transform<T, U = T> = {
  readonly input: DeepReadonly<T>;
  readonly output: DeepReadonly<U>;
  readonly transformed: boolean;
};

export function utility42<T>(value: T): Utility42Result<T> {
  return value as Utility42Result<T>;
}

export function transformUtility42<T, U = T>(
  input: T,
  transform?: (value: DeepReadonly<T>) => U
): Utility42Transform<T, U> {
  return {
