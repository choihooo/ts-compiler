...
/**
 * Additional utility type 2
 */

export type Utility2Base<T> = {
  readonly value: DeepReadonly<T>;
  readonly path?: Path<T>;
};

export type Utility2Options<T = any> = DeepPartial<Utility2Base<T>>;

export type Utility2Result<T> = IsPromise<T> extends true
  ? Promise<DeepReadonly<PromiseValue<T>>>
  : DeepReadonly<T>;

export type Utility2Transform<T, U = T> = {
  readonly input: DeepReadonly<T>;
  readonly output: DeepReadonly<U>;
  readonly transformed: boolean;
};

export function utility2<T>(value: T): Utility2Result<T> {
  return value as Utility2Result<T>;
}

export function transformUtility2<T, U = T>(
  input: T,
  transform?: (value: DeepReadonly<T>) => U
): Utility2Transform<T, U> {
  return {
