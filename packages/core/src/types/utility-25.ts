...
/**
 * Additional utility type 25
 */

export type Utility25Base<T> = {
  readonly value: DeepReadonly<T>;
  readonly path?: Path<T>;
};

export type Utility25Options<T = any> = DeepPartial<Utility25Base<T>>;

export type Utility25Result<T> = IsPromise<T> extends true
  ? Promise<DeepReadonly<PromiseValue<T>>>
  : DeepReadonly<T>;

export type Utility25Transform<T, U = T> = {
  readonly input: DeepReadonly<T>;
  readonly output: DeepReadonly<U>;
  readonly transformed: boolean;
};

export function utility25<T>(value: T): Utility25Result<T> {
  return value as Utility25Result<T>;
}

export function transformUtility25<T, U = T>(
  input: T,
  transform?: (value: DeepReadonly<T>) => U
): Utility25Transform<T, U> {
  return {
