...
/**
 * Additional utility type 36
 */

export type Utility36Base<T> = {
  readonly value: DeepReadonly<T>;
  readonly path?: Path<T>;
};

export type Utility36Options<T = any> = DeepPartial<Utility36Base<T>>;

export type Utility36Result<T> = IsPromise<T> extends true
  ? Promise<DeepReadonly<PromiseValue<T>>>
  : DeepReadonly<T>;

export type Utility36Transform<T, U = T> = {
  readonly input: DeepReadonly<T>;
  readonly output: DeepReadonly<U>;
  readonly transformed: boolean;
};

export function utility36<T>(value: T): Utility36Result<T> {
  return value as Utility36Result<T>;
}

export function transformUtility36<T, U = T>(
  input: T,
  transform?: (value: DeepReadonly<T>) => U
): Utility36Transform<T, U> {
  return {
