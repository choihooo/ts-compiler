...
/**
 * Additional utility type 13
 */

export type Utility13Base<T> = {
  readonly value: DeepReadonly<T>;
  readonly path?: Path<T>;
};

export type Utility13Options<T = any> = DeepPartial<Utility13Base<T>>;

export type Utility13Result<T> = IsPromise<T> extends true
  ? Promise<DeepReadonly<PromiseValue<T>>>
  : DeepReadonly<T>;

export type Utility13Transform<T, U = T> = {
  readonly input: DeepReadonly<T>;
  readonly output: DeepReadonly<U>;
  readonly transformed: boolean;
};

export function utility13<T>(value: T): Utility13Result<T> {
  return value as Utility13Result<T>;
}

export function transformUtility13<T, U = T>(
  input: T,
  transform?: (value: DeepReadonly<T>) => U
): Utility13Transform<T, U> {
  return {
